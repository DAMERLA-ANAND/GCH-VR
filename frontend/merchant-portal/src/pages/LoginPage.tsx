...
export default function LoginPage({ onLogin }: { onLogin?: (user: any) => void }) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('drp_merchant_user', JSON.stringify(res));
    ...
    <div className="min-h-[80vh] flex items-center justify-center p-6">
      <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 p-8 rounded-2xl shadow-2xl max-w-md w-full space-y-6">
        <div className="text-center space-y-2">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-indigo-600 to-indigo-400 mx-auto flex items-center justify-center font-black text-white text-xl shadow-lg shadow-indigo-500/30">
            M
          </div>
          <h1 className="text-2xl font-extrabold text-white">Merchant & Admin Portal Login</h1>
          <p className="text-xs text-slate-400">Sign in to manage chargebacks, rules, and admin operations</p>
        </div>
        ...
        <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800/80 text-[11px] text-slate-400 space-y-1.5">
          <span className="font-bold text-slate-300 block">Role Credentials:</span>
          <p>• <strong>Merchant</strong>: <code className="text-indigo-300 font-mono">merchant</code> / <code className="text-indigo-300 font-mono">password123</code></p>
          <p>• <strong>Platform Admin</strong>: <code className="text-indigo-300 font-mono">admin</code> / <code className="text-indigo-300 font-mono">admin123</code></p>
        </div>
      </div>
    </div>
  );
